...
public class DefaultPropositionGenerator extends PropositionGenerator {
	...
	public void generate(List<Proposition> result, Clause clause,
		...
		System.out.println("subject proposition="+proposition);
		// process verb
		...
		System.out.println("verb proposition="+proposition);
		...
		System.out.println("sortedIndexes="+sortedIndexes
						+" clause="+clause
						+" propostion="+propositions);
		for(Integer index: sortedIndexes) {
			if (clause.constituents.get(clause.verb) instanceof IndexedConstituent
					&& clause.adverbials.contains(index) && 
					((IndexedConstituent)clause.constituents.get(index)).getRoot().index()
					< ((IndexedConstituent)clause.constituents.get(clause.verb)).getRoot().index()) 
						continue;
